...
namespace Drupal\edw_paragraphs\Plugin\Field\FieldWidget;
 ...
 *
 * Inspired from 'OpenEuropa Paragraphs with variants' widget.
 *
 * @FieldWidget(
 *   id = "edw_paragraphs_variants",
...
class ParagraphVariantsWidget extends ParagraphsWidget {
  ...
  /**
   * Constructs a ParagraphVariantsWidget object.
   *
   ...
   *   The current user.
   */
  public function __construct($plugin_id, $plugin_definition, FieldDefinitionInterface $field_definition, array $settings, array $third_party_settings, EntityTypeManagerInterface $entityTypeManager, ContentTranslationManagerInterface $manager, ModuleHandlerInterface $moduleHandler, EntityTypeBundleInfoInterface $entityTypeBundleInfo, EntityDisplayRepositoryInterface $displayRepository, AccountInterface $currentUser) {
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    /** @var \Drupal\Core\Entity\ContentEntityBase $host */
    ...
    elseif (isset($widget_state['selected_bundle'])) {
      ...
      /** @var \Drupal\paragraphs\Entity\Paragraph $paragraphs_entity */
      $paragraphs_entity = $this->entityTypeManager->getStorage($target_type)->create([
    ...
    if ($paragraphs_entity) {
      ...
      if (!$this->isTranslating) {
        ...
        if (!$paragraphs_entity->hasTranslation($langcode)) {
          ...
          if ($paragraphs_entity->hasField('content_translation_source')) {
            ...
            $this->manager->getTranslationMetadata($translation)->setSource($paragraphs_entity->language()->getId());
          }
        }
      ...
      if ($this->moduleHandler->moduleExists('content_translation')) {
        $settings = $this->manager->getBundleTranslationSettings('paragraph', $paragraphs_entity->getParagraphType()->id());
      ...
      if (isset($item_bundles[$paragraphs_entity->bundle()])) {
        ...
        $this->moduleHandler->alter('paragraphs_widget_actions', $widget_actions, $context);
      ...
      if (isset($widget_state['paragraphs'][$delta]['display'])) {
        ...
        $display = EntityFormDisplay::collectRenderDisplay(
          $paragraphs_entity,
          $paragraphs_entity->get('edw_paragraphs_variant')->value ?? $this->getSetting('form_display_mode')
        );
      }
      ...
      if ($item_mode == 'edit') {
        ...
        if ($paragraphs_type && $this->currentUser->hasPermission('edit behavior plugin settings') && (!$this->isTranslating || !$hide_untranslatable_fields)) {
      ...
      if ($item_mode === 'edit') {
        $view_modes = $this->displayRepository->getFormModeOptionsByBundle($paragraphs_entity->getEntityTypeId(), $paragraphs_entity->bundle());
  ...
  public function massageFormValues(array $values, array $form, FormStateInterface $form_state): array {
    ...
    foreach ($values as $delta => $item) {
      if (isset($values[$delta]['entity']) && isset($item['variant'])) {
        ...
        $values[$delta]['entity']->set('edw_paragraphs_variant', $item['variant'] ?? 'default');
      }
    }
